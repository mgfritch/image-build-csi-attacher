...
ARG PKG=github.com/kubernetes-csi/external-attacher
ARG TAG
...
ARG TARGETARCH
RUN xx-go --wrap && \
    go-build-static.sh -gcflags=-trimpath=${GOPATH}/src -o "/usr/local/bin/csi-attacher" ./cmd/csi-attacher
RUN xx-verify --static /usr/local/bin/csi-attacher
RUN if [ "$(xx-info arch)" = "amd64" ]; then \
        go-assert-boring.sh /usr/local/bin/csi-attacher; \
    fi
